...
int32_t getProductIndex(const GUID* guid, const PVlmcsdData_t list, const int32_t count, const char** name, const char** ePid)
{
	int i;
	...
	for (i = count - 1; i >= 0; i--)
	{
		if (IsEqualGUID(guid, &list[i].Guid))
		{
			...
			if (ePid) *ePid = list[i].EPid;
			return i;
		}
	}

	if (name) *name = "Unknown";
	if (ePid) *ePid = KmsData->EPidWindows;
	return i;
}
...
static void logRequest(const REQUEST *const baseRequest)
{
	...
	index = getProductIndex(&baseRequest->ActID, KmsData->SkuItemList, KmsData->SkuItemCount, &productName, NULL);
	if (index < 0) index = getProductIndex(&baseRequest->KMSID, KmsData->KmsItemList, KmsData->KmsItemCount, &productName, NULL);
	if (index < 0) index = getProductIndex(&baseRequest->AppID, KmsData->AppItemList, KmsData->AppItemCount, &productName, NULL);
...
#	ifndef NO_STRICT_MODES
	int32_t index =
#	endif
	getProductIndex(&baseRequest->KMSID, KmsData->KmsItemList, KmsData->KmsItemCount, NULL, &ePid);
...
#	ifndef NO_CLIENT_LIST
	int32_t appIndex = APP_ID_WINDOWS;
#	endif // NO_CLIENT_LIST

#	endif // !NO_STRICT_MODES

	int32_t ePidIndex = EPID_INDEX_WINDOWS;

	if (ePid == KmsData->EPidOffice2010)
	{
#		ifndef NO_CLIENT_LIST
		appIndex = APP_ID_OFFICE2010;
#		endif // NO_CLIENT_LIST
		ePidIndex = EPID_INDEX_OFFICE2010;
	}
	else if (ePid == KmsData->EPidOffice2013)
	{
#		ifndef NO_CLIENT_LIST
		appIndex = APP_ID_OFFICE2013;
#		endif // NO_CLIENT_LIST
		ePidIndex = EPID_INDEX_OFFICE2013;
	}
	else if (ePid == KmsData->EPidOffice2016)
	{
#		ifndef NO_CLIENT_LIST
		appIndex = APP_ID_OFFICE2013;
#		endif // NO_CLIENT_LIST
		ePidIndex = EPID_INDEX_OFFICE2016;
	}
...
#	endif // !defined(NO_STRICT_MODES)
	{
		DWORD minimum_answer_clients = ePid != KmsData->EPidWindows ? 10 : 50;
