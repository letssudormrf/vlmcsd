...
static void dataFileFormatError()
{
...
void loadKmsData()
{
	long size = (long)getDefaultKmsDataSize();
...
#	endif // DATA_FILE
	...
	if (strcmp(fn_data, "-"))
	{
		...
		if (!file)
		{
			if (ExplicitDataLoad) dataFileReadError();
		}
		else
		{
			if (fseek(file, 0, SEEK_END)) dataFileReadError();
			size = ftell(file);
			if (size == -1) dataFileReadError();
			...
			if ((uint32_t)bytesRead != (uint32_t)size) dataFileReadError();
			fclose(file);
...
#	endif // NO_EXTERNAL_DATA
	...
	for (i = 0; i < totalItemCount; i++)
	{
		...
		item->EPid = (char*)KmsData + LE64(item->EPidOffset);
		item->Name = (char*)KmsData + LE64(item->NameOffset);

#		ifndef UNSAFE_DATA_LOAD
		if (
			item->EPid >= (char*)KmsData + (uint64_t)size ||
